...
class VehTestReportController extends Controller
{
    ...
    /**
     * แปลงวันที่จากพุทธศักราชเป็นคริสตศักราช
     * 
     * @param string $buddhistDate วันที่ในรูปแบบ พ.ศ. (Y-m-d)
     * @return string วันที่ในรูปแบบ ค.ศ. (Y-m-d)
     */
    protected function convertToGregorianDate($buddhistDate)
    {
        ...
        // แยกส่วนของวันที่
        $dateParts = explode('-', $buddhistDate);
        
        // ถ้าวันที่ไม่ได้อยู่ในรูปแบบที่ถูกต้อง
        if (count($dateParts) !== 3) {
            return $buddhistDate;
        }
        
        // ลบ 543 จากปี พ.ศ. เพื่อแปลงเป็น ค.ศ.
        $gregorianYear = intval($dateParts[0]) - 543;
        
        // สร้างวันที่ใหม่ในรูปแบบ ค.ศ.
        return $gregorianYear . '-' . $dateParts[1] . '-' . $dateParts[2];
    }
        ...
        if ($request->isPost) {
            ...
            try {
                ...
                foreach ($reportDates['reportDates'] as $index => $reportDate) {
                    ...
                    $reportDateSql = $this->convertToGregorianDate($reportDateSql);
